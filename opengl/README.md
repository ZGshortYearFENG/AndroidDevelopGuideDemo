...
![avatar](https://learnopengl-cn.readthedocs.io/zh/latest/img/01/04/pipeline.png)

3D坐标转变为适应你屏幕的2D像素的处理过程是由OpenGL的图形渲染管线管理的，图形渲染管线可以被划分为两个主要部分：第一部分把你的3D坐标转换为2D坐标，第二部分是把2D坐标转变为实际的有颜色的像素  
